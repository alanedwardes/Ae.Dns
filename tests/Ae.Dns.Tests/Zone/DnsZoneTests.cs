...
namespace Ae.Dns.Tests.Zone
{
    ...
    {
        private IDnsZone _wikipediaExampleZone = new DnsZone
        {
            Origin = "example.com",
        ...
        public async Task TestLoadZone1()
        {
            ...
            DnsZoneSerializer.DeserializeZone(zone, await File.ReadAllTextAsync("Zone/test1.zone"));
        ...
        {
            var zone = new DnsZone();
            DnsZoneSerializer.DeserializeZone(zone, await File.ReadAllTextAsync("Zone/test2.zone"));
        }
        ...
        public async Task TestLoadZone3()
        {
            ...
            var zone = new DnsZone { Records = message.Nameservers };
        ...
        public async Task<string> RoundTripRecords(params DnsResourceRecord[] records)
        {
            ...
            await originalZone.Update(() =>
            {
                foreach (var record in records)
                {
                    originalZone.Records.Add(record);
                }

                return true;
            });

            var serialized = DnsZoneSerializer.SerializeZone(originalZone);

            var newZone = new DnsZone();
            DnsZoneSerializer.DeserializeZone(newZone, serialized);

            Assert.Equal(originalZone.Records, newZone.Records);

            return DnsZoneSerializer.SerializeZone(newZone);
        }
    }
}
